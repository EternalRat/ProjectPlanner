...
const apiClass = {
    ...
    async postTodo(cookies: any, todo: Todo) {
        return await axios.post(`${process.env.REACT_APP_API_URL}/todo`, {
            label: todo.label,
    ...
    async updateTodo(cookies: any, todo: Todo) {
        return await axios.patch(`${process.env.REACT_APP_API_URL}/todo`, {
            id: todo.id,
            label: todo.label,
            description: todo.description,
            limitDate: todo.limitDate,
    ...
    async deleteTodo(cookies: any, todo: Todo) {
        return await axios.delete(`${process.env.REACT_APP_API_URL}/todo/${todo.id}`, {
            headers: {
                authorization: cookies.token
            }
        });
